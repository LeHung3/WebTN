    ...
    if (exam) {
        examDetailsContainer.innerHTML = `
            ...
            <form id="editExamForm">
                ...
                <button type="button" class="btn btn-primary btn-sm" onclick="addQuestions()">Thêm câu hỏi</button>
                <hr>
        ...
        for (var i = 1; i <= questionCount; i++) {
            ...
            <div id="${exam.questions[i-1].questionId}">
                ...
                <div class="form-group">
                    <button type="button" class="btn btn-danger btn-sm" onclick="deleteQuestion(${exam.questions[i-1].questionId})">Xóa</button>
                </div>
            </div>
            `;
        ...
        examDetailsContainer.innerHTML=""
        alert('Không tìm thấy kỳ thi với Exam ID đã nhập.');
    }
});
...
document.getElementById('examDetails').addEventListener('submit', function(event) {
    event.preventDefault();
